...
export default function Dashboard() {
  ...
  const headerStats = [
    { name: 'Mensajes totales', value: logs.length, icon: MessageSquare, color: 'text-zinc-950 ' },
    { name: 'Usuarios únicos', value: new Set(logs.map(l => l.session_id)).size, icon: Users, color: 'text-zinc-950 ' },
    { name: 'Acciones Tracking', value: (stats?.categories || []).reduce((acc, curr) => acc + Number(curr.value), 0), icon: Target, color: 'text-zinc-950 ' },
  ];
  ...
  return (
    <div className="flex min-h-screen bg-[#fafafa]  text-zinc-900  font-sans overflow-hidden">
      ...
      <aside className={cn(
        ...
        <div className="p-6 border-b border-zinc-100 ">
          <div className="flex items-center justify-between">
            ...
            <button className="lg:hidden p-2 hover:bg-zinc-100 :bg-zinc-800 rounded-lg transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
          ...
          <button
            ...
            className={cn(
              ...
              activeTab === 'overview'
                ...
                : "text-zinc-600  hover:bg-zinc-100 :bg-zinc-800 hover:text-black :text-white"
            )}
          >
        ...
        <div className="p-4 border-t border-zinc-100  bg-zinc-50/50 ">
          <button
            onClick={() => signOut()}
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold text-red-600  hover:bg-red-50 :bg-red-950/30 transition-all group"
          >
        ...
        <header className="flex justify-between items-center p-6 lg:p-10 border-b border-zinc-100  bg-[#fafafa]/80  backdrop-blur-md sticky top-0 z-30">
          <div>
            <div className="flex items-center gap-3 lg:hidden mb-2">
              <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 -ml-2 hover:bg-zinc-100 :bg-zinc-800 rounded-lg">
            ...
            <p className="text-sm text-zinc-500  hidden lg:block">Monitoriza y gestiona tu sistema de IA.</p>
          </div>
          <button
            onClick={fetchData}
            className="group flex items-center gap-2 bg-white  border border-zinc-200  px-4 py-2.5 rounded-xl text-xs lg:text-sm font-bold hover:border-black :border-white transition-all shadow-sm"
          >
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-zinc-100 ">
                      {logs.slice(0, 5).map((log) => (
                        <tr key={log.id} className="hover:bg-zinc-50 :bg-zinc-800/30 transition-all group">
                          <td className="px-8 py-6">
                            <span className={cn(
                              "px-3 py-1.5 rounded-full text-[10px] font-black tracking-tighter",
                              log.sender === 'user' ? "bg-blue-50 text-blue-600  " : "bg-black text-white   shadow-sm"
                            )}>
              ...
              <div className="grid grid-cols-1 gap-4">
                {sessions.map((session) => (
                  <button
                    ...
                    className="flex items-center justify-between p-6 bg-white  border border-zinc-200  rounded-3xl hover:border-black :border-white transition-all text-left group shadow-sm hover:shadow-md"
                  >
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-3 mt-2">
                          ...
                          <span className="bg-zinc-100  px-2 py-0.5 rounded text-[9px] font-black uppercase">
                            {session.count} Mensajes
                          </span>
                        </div>
                      </div>
                    </div>
                    <ChevronRight className="w-5 h-5 text-zinc-300 group-hover:text-black :text-white transition-colors" />
                  </button>
                ))}
              </div>
            </div>
      ...
      <aside className={cn(
        ...
        {selectedSessionId && (
          <>
            <div className="p-6 border-b border-zinc-100  flex items-center justify-between bg-white  backdrop-blur-md sticky top-0">
              <div>
              ...
              <button
                ...
                className="p-3 bg-zinc-100  rounded-2xl hover:bg-black hover:text-white :bg-white :text-black transition-all"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
